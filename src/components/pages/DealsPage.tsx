...
function DealCard({
 ...
 return (
 <div className="group bg-card shadow-sm border border-border/50 rounded-xl p-4 hover:bg-card shadow-sm hover:shadow-lg hover:border-border/50 transition-[background-color,box-shadow,border-color] duration-200">
 ...
 {deal.notities && (
 <div className="mt-2 text-xs text-muted-foreground bg-muted rounded-md p-2 max-h-16 overflow-y-auto">
 {deal.notities}
 </div>
 )}
 </div>
...
"flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium transition-all",
 ...
 <div key={stat.label} className="bg-card shadow-sm border border-border/50 rounded-xl p-4 flex items-center gap-3">
 ...
 <div className={cn('flex items-center justify-between p-3 rounded-t-xl bg-linear-to-r border border-border/50', meta.gradient)}>
 ...
 <div className="bg-card shadow-sm border border-t-0 border-border/50 rounded-b-xl p-3 space-y-3 min-h-[220px] max-h-[calc(100vh-360px)] overflow-y-auto custom-scrollbar">
 {loading && (
 Array.from({ length: 3 }).map((_, index) => (
 <div key={`${stage}-loading-${index}`} className="h-28 rounded-xl bg-muted animate-pulse"/>
 ))
 )}
